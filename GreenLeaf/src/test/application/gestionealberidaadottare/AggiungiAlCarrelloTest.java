...
public class AggiungiAlCarrelloTest extends Mockito {
  ...
  private static final HttpServletResponse httpServletResponse =
  ...
  public UtenteBean utenteBean = new UtenteBean("test6@gmail.com", "Angelo99", "Afeltra", "Angelo",
  ...
  {
    try {
      associati = (ArrayList<AssociatoBean>) associatoDao.doRetrieveAlbero(
          categoriaBean.getNomeCategoria());
  ...
  public void tc_aact_1() throws ServletException, IOException {
    ...
    when(httpServletRequest.getParameter("scelta")).thenReturn(
        associati.get(1).getRegioneAssociato());
    when(httpServletRequest.getParameter("categoria")).thenReturn(categoriaBean.getNomeCategoria());
    when(httpServletRequest.getContextPath()).thenReturn(
        "http://localhost:8080/GreenLeaf_war_exploded");
